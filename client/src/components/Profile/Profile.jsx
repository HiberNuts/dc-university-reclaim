...
const Profile = ({ isOpen, closeModal }) => {
    ...
    const fetchUserData = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_BACKEND_URL}/auth/getUserData?userid=${
    ...
    if (formData.designation === 'Select Designation') {
      ...
      try {
        const response = await axios.put(
          `${import.meta.env.VITE_BACKEND_URL}/auth/update?userid=${
            loggedInUserData._id
          }`,
          {
            ...formData,
  ...
  const handleResendVerificationEmail = async () => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/auth/resend?userId=${
          loggedInUserData._id
        }`,
        {
  ...
  return (
    <div className="w-full font-helvetica-neue  h-full flex justify-between align-middle">
      ...
      >
        <div className="bg-shardeumBlue px-14 lg:w-[25%] h-[100vh]   left-0 rounded-br-xl  flex flex-col align-middle items-center">
          <div className="text-white mt-20">
            ...
            />
            {isEditing === false && (
              <p className="text-center text-[22px] font-[700] mt-2 ">
                {formData.name}
              </p>
            )}
          </div>
          {isEditing ? (
                      ...
                      <Listbox.Button className="relative w-full flex-row cursor-default rounded-lg bg-gray-50 py-2 text-left border border-gray-300 text-gray-900 text-sm focus:ring-shardeumOrange focus:border-shardeumOrange shadow-md focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white/75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 bg-white">
                        <div className="px-2 flex justify-between align-middle h-full w-full">
                          <span className="block truncate">
                            {formData.designation || 'Web3 Beginner'}
                          </span>
            ...
            <div
              className={`w-full mt-6 flex flex-col h-[50%] justify-start gap-16 align-middle items-center`}
            >
              ...
              <div className="w-[100%] justify-center align-middle flex">
                {/* <button
                  ...
                  className={`bg-white flex justify-center align-middle hover.bg-[#fc7d34] rounded-[10px] transition ease-in-out items-center font-semibold text-center text-white text-[22px] w-[100%] h-[40px] `}
                >
                  <div className="flex h-[100%] w-full justify-between px-2">
                    <div className='self-center'>
                      <p className="text-shardeumBlue ">Edit Profile</p>
                    </div>

                    

                      <img
                        className="h-[50%] text-shardeumBlue self-center"
                        src={pencil}
                        alt="pencil"
                      />
          ...
          <div className="content  w-[full] mx-20">
            <div className="mb-14">
              ...
              <span className="font-helvetica-neue text-[18px] font-[500]">
                What a wonderful day it is to explore new knowledge. Today brims
                with possibilities and fresh knowledge waiting to be discovered.
